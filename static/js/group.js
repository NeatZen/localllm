...
const GROUP_STATE_KEY = 'neatai-group-state';
...
export async function startGroup(models, parentSessionId) {
  ...
  try {
    ...
    try {
      const gids = JSON.parse(localStorage.getItem('neatai-group-sessions') || '[]');
      if (!gids.includes(_parentSessionId)) { gids.push(_parentSessionId); localStorage.setItem('neatai-group-sessions', JSON.stringify(gids)); }
    } catch (e) {}
  } catch (e) {
