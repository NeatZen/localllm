...
const _ACTION_RECENTS_KEY = 'neatai-msg-actions-recent';
 ...
 */
const _USER_ACTION_RECENTS_KEY = 'neatai-user-actions-recent';
    ...
    ctxRing.addEventListener('click', (e) => {
      ...
      if (compactBtn) {
        compactBtn.addEventListener('click', async (e) => {
          ...
          compactRole.textContent = 'NeatAi';
...
export function addMessage(role, content, modelName, metadata) {
  try {
    ...
    var _roleText = role === 'user' ? 'You' : (isSlash || isCompacted) ? 'NeatAi' : shortModel(resolvedModel);
