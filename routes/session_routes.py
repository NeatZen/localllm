    ...
    def list_sessions(request: Request):
        ...
        try:
            ...
            rows = db.query(
                ...
                DbSession.total_output_tokens, DbSession.is_important, DbSession.created_at,
                DbSession.updated_at, DbSession.last_message_at, DbSession.mode,
                DbSession.message_count, DbSession.workspace_project_id,
            ...
            for row in rows:
                ...
                ws_project_map[row.id] = row.workspace_project_id
            ...
            # Include ALL DB sessions for this user (not just in-memory cache).
            # Workspace sessions are created empty and must appear immediately.
            q = db.query(DbSession).filter(DbSession.archived == False)
            q = owner_filter(q, DbSession, user)
            db_session_rows = q.all()
        finally:
            db.close()
        ...
        sessions = [{"id": s.id, "name": s.name, "model": s.model,
                     ...
                     "workspace_project_id": ws_project_map.get(s.id) or s.workspace_project_id,
                     "message_count": msg_count_map.get(s.id, 0)}
                    for s in db_session_rows
                    if (s.name or "").strip() not in ("Nobody", "Incognito")]
