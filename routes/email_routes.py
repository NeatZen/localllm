...
NEATAIEUS_MAIL_ORIGIN = "neatai-ui"
...
def _apply_neatai_headers(msg, kind: str | None = None, ref_id: str | None = None):
    msg["X-NeatAi-Origin"] = NEATAIEUS_MAIL_ORIGIN
    if kind:
        msg["X-NeatAi-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", kind)[:64]
    if ref_id:
        msg["X-NeatAi-Ref"] = re.sub(r"[^A-Za-z0-9_.:-]", "-", ref_id)[:128]
...
def setup_email_routes():
    ...
    def _list_emails_sync(folder, limit, offset, filter_, account_id, from_addr=None, has_attachments_only=False, owner=""):
        ...
        try:
            ...
            if filter_ == "unread":
                ...
                # Prefer the NeatAi marker header, but include the subject
                # fallback too. The fallback uses a distinct NeatAi prefix
                # so ordinary emails containing "Reminder" don't get mixed in.
                status, data = conn.search(
                    None,
                    f'(OR HEADER X-NeatAi-Kind "reminder" SUBJECT "Reminder (NeatAi):"{from_clause})',
                )
    ...
    @router.delete("/neatai/reminders")
    async def delete_neatai_reminder_emails(
        account_id: str | None = Query(None),
        permanent: bool = Query(False),
        owner: str = Depends(require_owner),
    ):
        """Delete email messages stamped as NeatAi reminders."""
        if account_id:
        ...
        try:
            ...
            with _imap(account_id, owner=owner) as conn:
                ...
                for folder_name in candidates:
                    ...
                    try:
                        ...
                        uids.update(_search_uids(conn, f'(HEADER X-NeatAi-Kind {_search_quote("reminder")})'))
                        uids.update(_search_uids(conn, f'(SUBJECT {_search_quote("Reminder (NeatAi):")})'))
                        for addr in own_addrs:
                            addr_q = _search_quote(addr)
                            uids.update(_search_uids(conn, f'(FROM {addr_q} SUBJECT {_search_quote("Reminder (NeatAi):")})'))
                            # Legacy reminders created before the NeatAi
            ...
            logger.error(f"delete_neatai_reminder_emails failed: {e}")
            return {"success": False, "error": "Mail operation failed"}
    ...
    async def _send_email_sync(
        ...
        account_id=None, owner="", neatai_kind=None, neatai_ref=None,
    ):
        ...
        _apply_neatai_headers(outer, neatai_kind or "scheduled", neatai_ref)
        if in_reply_to:
    ...
    async def schedule_email(req: dict, owner: str = Depends(require_owner)):
        ...
        try:
            ...
            conn.execute("""
                ...
                (id, to_addr, cc, bcc, subject, body, in_reply_to, references_hdr, attachments, send_at, created_at, status, account_id, neatai_kind)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?)
            """, (
                sid,
                req.get("to", ""),
                ...
                req.get("neatai_kind") or "scheduled",
            ))
            conn.commit()
            conn.close()
    ...
    async def send_email(req: SendEmailRequest, background_tasks: BackgroundTasks, owner: str = Depends(require_owner)):
        ...
        outer["Date"] = datetime.utcnow().strftime("%a, %d %b %Y %H:%M:%S +0000")
        outer["Message-ID"] = email.utils.make_msgid(domain="neatai.local")
        ...
        if req.neatai_kind:
            _apply_neatai_headers(outer, req.neatai_kind)
