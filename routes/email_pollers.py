                        ...
                        if jm:
                            ...
                            if urgency in ("critical", "high"):
                                try:
                                    ...
                                    # Deep-link to open the original email in NeatAi (if public URL is configured).
                                    ...
                                    # HTML alternative with a clickable "Open in NeatAi" button
                                    import html as _h
...
def _scheduled_poll_once() -> dict:
    ...
    try:
        ...
        kind_expr = "neatai_kind" if "neatai_kind" in cols else "'scheduled' AS neatai_kind"
        rows = conn.execute(f"""
        ...
        for r in rows:
            ...
            try:
                ...
                neatai_kind = r[10] if len(r) > 10 else "scheduled"
                ...
                outer["X-NeatAi-Origin"] = "neatai-ui"
                outer["X-NeatAi-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", neatai_kind or "scheduled")[:64]
                outer["X-NeatAi-Ref"] = sid
                if r[6]:
...
async def _scheduled_email_poller():
    ...
    also exposed via the `neatai-mail poll-scheduled` CLI for
...
def _inprocess_pollers_enabled() -> bool:
    """Honour `NEATAIEUS_INPROCESS_POLLERS` — set to `0`/`false`/`no`/`off`
    to disable the asyncio tasks so a cron / systemd-timer setup driving
    `neatai-mail poll-scheduled` is the sole external driver. The legacy
    ...
    import os
    raw = os.environ.get("NEATAIEUS_INPROCESS_POLLERS", "1").strip().lower()
...
def _start_poller():
    ...
    Skipped entirely when `NEATAIEUS_INPROCESS_POLLERS=0` — use that when
    ...
    if not _inprocess_pollers_enabled():
        logger.info(
            "In-process email pollers disabled (NEATAIEUS_INPROCESS_POLLERS=0); "
            "drive `neatai-mail poll-scheduled` externally."
        )
        return
    import asyncio
