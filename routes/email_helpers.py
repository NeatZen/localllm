...
# Override at deploy time via NEATAIEUS_MAIL_ATTACHMENTS_DIR. Defaults to a
# subdir of the install's data/ tree so the app works out-of-the-box without
# a hardcoded /home/<user>/ path.
ATTACHMENTS_DIR = Path(os.environ.get("NEATAIEUS_MAIL_ATTACHMENTS_DIR", str(DATA_DIR / "mail-attachments")))
...
def _init_scheduled_db():
    ...
    try:
        ...
        if "neatai_kind" not in cols:
            conn.execute("ALTER TABLE scheduled_emails ADD COLUMN neatai_kind TEXT")
    except Exception:
        pass
...
class SendEmailRequest(BaseModel):
    ...
    # Internal marker for NeatAi-generated mail (e.g. reminder, scheduled).
    neatai_kind: Optional[str] = None
