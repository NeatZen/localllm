...
async def _generate_tmux(cmd: str, request: Request):
    ...
    script_path.write_text(
        f"#!/bin/bash\n"
        f"NEATAIEUS_USER_SHELL=\"${{SHELL:-}}\"\n"
        f"if [ -n \"$NEATAIEUS_USER_SHELL\" ] && [ -x \"$NEATAIEUS_USER_SHELL\" ]; then\n"
        f"  NEATAIEUS_USER_PATH=\"$(\"$NEATAIEUS_USER_SHELL\" -ic 'printf \"__NEATAIEUS_PATH__%s\\n\" \"$PATH\"' 2>/dev/null | sed -n 's/^__NEATAIEUS_PATH__//p' | tail -n 1 || true)\"\n"
        f"  if [ -n \"$NEATAIEUS_USER_PATH\" ]; then export PATH=\"$NEATAIEUS_USER_PATH:$PATH\"; fi\n"
        f"fi\n"
