...
def test_inprocess_pollers_gate(monkeypatch):
    """The NEATAIEUS_INPROCESS_POLLERS env var must let operators kill
    the asyncio pollers when cron / systemd is driving the one-shot
    `neatai-mail poll-*` CLI subcommands instead. Two pollers racing
    ...
    monkeypatch.delenv("NEATAIEUS_INPROCESS_POLLERS", raising=False)
    assert _inprocess_pollers_enabled() is True

    # Any of the off-values disables.
    for off in ("0", "false", "no", "off", "FALSE", "Off"):
        monkeypatch.setenv("NEATAIEUS_INPROCESS_POLLERS", off)
    ...
    for on in ("1", "true", "yes", "anything-truthy"):
        monkeypatch.setenv("NEATAIEUS_INPROCESS_POLLERS", on)
...
def _load_search_content_for_test(monkeypatch, name="services.search.content_under_test"):
    ...
    cache.CONTENT_CACHE_DIR = Path("/tmp/neatai-test-content-cache")
